function [Dir_Name,recon_def] = radial_recon_01July2025(path,traj,Method_Params,Config_Params,fast_recon)
...
% Samal - Hard Coded the cut off, to just get the first FID
FIDs = FIDs(1:70, :);

disp('Reading Trajectory File')
traj_File = dir('traj');
traj = Bruker_Load_Traj(traj_File.name);
traj = reshape(traj, [3,size(FIDs,1),size(FIDs,2)]);
...
%             load(['RetroGating' num2str(i) '.mat'])
%         end
        n = size(traj,3);     % total size
        m = 1;      % start index
        p = size(FIDs,2);      % end index
        
        Exp_indx = false(n,1);  % create 1xN logical matrix of all false
        Exp_indx(m:p) = true;    % set values from m to p to true
        Insp_indx = Exp_indx;
