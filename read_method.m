...
if Length_X > 1
    ...
    if strcmp(Seq,'Radial')  %Calculate Theoretical Trajectories for Radial
        ...
        if strcmp(FlybackYN,'No')
            Tot_Num_Pts = NumPts + RampPoints;
        else
...
elseif strcmp(Seq,'Spiral')
    ...
    for index = (SzStart+1):(EchoTimesEnd-1) %For the spiral sequence, The AcqShift Line is the cutoff - was previously labeled EchoTimesEnd... maybe go back and change this at some point
        readOutStr = methodRead{index};
    ...
    %radius = squeeze(sqrt(traj(1,:,:).^2+traj(2,:,:).^2+traj(3,:,:).^2));
    %maxradius = max(max(radius));
    %traj = traj/maxradius/2.2223;
    %Need to scale the resolution? Maybe... Let's give this a try
    %res_sphere = 0.805995977008253;
