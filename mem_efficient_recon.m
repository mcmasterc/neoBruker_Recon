function [Image_Out,kspace,Recon_Params] = mem_efficient_recon(ImageSize,data,traj)
...
nDcfIter = 10;
deapodizeImage = false; %set to true -- BA_06Aug2019
%apodizeKspace = ture;
%deapodizeImage = true;
nThreads = 10;
...
kernel.sharpness = [0.3 0.3 0.3];
kernel.extent = [9*kernel.sharpness 9*kernel.sharpness 9*kernel.sharpness];
systemObj = MemoryEfficientThreadedGridder(traj,ImageSize,kernel.sharpness,kernel.extent,overgrid_factor,nThreads);
